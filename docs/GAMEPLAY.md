...
**Zoomolható/pásztázható felület**: a fa nagyobb, mint a képernyő (a node-ok közti táv 240px, minden node egyforma méretű, 56px átmérő). A `CanvasLayer/SkillTreeViewport` (`clip_contents=true`) alatti `SkillTreeCanvas` Control tartalmazza az összes node-ot és él-vonalat. Induláskor a teljes fa látszik (kicsinyítve, középre igazítva, `MainMenu.FitSkillTreeToView`), onnantól görgővel zoomolható (a kurzor alatti pont marad a helyén), bal VAGY középső gomb húzásával pásztázható (`MainMenu._Input`) — a bal gombos húzás egy kis elmozdulási küszöbbel (`PanDragThreshold`) különbözteti meg a node-kattintástól.

**Élvonalak mint elforgatott ColorRect-ek** (`MainMenu.BuildSkillTreeEdges`), NEM egyedi `_Draw()`-t hívó Control: egy korábbi verzió egyetlen custom-draw Control-lal (`SkillTreeEdgeLayer`) rajzolta az összes élt — ez zoomolás közben nem (vagy alig) látszott, feltehetően a `_Draw()`/`QueueRedraw()` és a dinamikusan változó szülő-transzform valamilyen összjátéka miatt (a pontos okot nem sikerült kideríteni). A csere minden élhez egy elforgatott, vékony `ColorRect`-et hoz létre (`Position`=kezdőpont, `Size`=(hossz,4), `PivotOffset`=(0,2), `Rotation`=az irányszög) — ugyanaz a rendercsatorna, mint a gombok saját rajzolása, ami bizonyítottan helyesen követi a pan/zoom transzformot minden zoom-szinten.

**Elágazási szög korlátozva** (`SkillTreeAngleStep = 10°`): az elágazások/kanyarok a szülő ág saját tengelyétől csak kis mértékben térnek el, sosem a szomszéd ág felé (a 4 ág 90°-ra van egymástól, a legrosszabb eset is max ~40°-ot tér el a saját tengelytől). Az első verzióban 90°-os kanyar + 45°-os elágazás volt, ami miatt pl. a Gold ág node-jai felfelé/lefelé is elkezdtek "kanyarodni" és átfedésbe kerültek a szomszédos ágakkal — ez volt a "borzalmas" (a felhasználó szava) átfedős kép oka.

**`SkillSegment` (node-ok közti táv) és a szög összefüggése**: egy elágazásnál a két testvér node távolsága kb. `2×SkillSegment×sin(SkillTreeAngleStep)` — ennek nagyobbnak KELL lennie, mint `NodeDiameter`, különben pont az elágazásoknál ér egymásba a két node. 145px szegmens + 10° csak ~50px távolságot adott (kevesebb, mint az 56px átmérő — ez okozta a második, "még mindig egymásba érnek" visszajelzést); 240px-re emelve ~83px lett a testvér-node távolság, kényelmes hézaggal. **A vonalak vastagsága/áttetszősége is nőtt** (2px/0,5 alfa → 4px/0,85 alfa) — a felhasználó jelezte, hogy közelről (nagy zoomnál) alig látszottak, pedig ez mutatja, melyik node melyikből ered.
