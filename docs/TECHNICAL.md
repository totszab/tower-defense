...
- **Torony-választék skill fa alapján**: nincs többé `[Export] PackedScene[] AvailableTowers` a scene-en — `LevelBuild.TowerUnlocks` egy kézzel karbantartott `(ScenePath, RequiredSkillNodeId)` lista (`null` = mindig elérhető), és `_Ready()` ebből építi fel a futásidejű `_availableTowers` listát a `PlayerProgress.GetSkillLevel(nodeId) >= 1` alapján — NEM a pálya-progressztől függ. A torony típusok **globálisan** elérhetők a megvásárlásuk után, minden pályán.
- **Pályánkénti vizuális téma**: `LevelBuild.ThemeFor(levelId)` ad grass/path csempeszínt + háttér-shader `color_a`/`color_b` párt. A háttér `ShaderMaterial`-t `_Ready()`-ben duplikálni kell (`.Duplicate()`) a paraméterek módosítása előtt — ugyanaz a megosztott-resource kockázat, mint az `Enemy` `CollisionShape2D`-jénél, mert a `.tscn`-ből jövő sub_resource elvileg cache-elődhet `ChangeSceneToFile` hívások között.
